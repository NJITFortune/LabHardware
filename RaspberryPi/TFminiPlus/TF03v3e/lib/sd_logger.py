...
class Logger:
    ...
    def __init__(self, headers:str=None,file_name:str=None, path:str="/sd/",batch_size:int=1000,overwrite:bool=False,bin:bool=False):
        if file_name:
    ...
    def save_recorded_data_to_file(self):  # fail safe for power lost - saving data on every self.batch_sizeth read
        self.file.close()
        if self.bin:
